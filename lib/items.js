...
class Items extends Component {
  ...
  render() {
    const { items, listHeight,styleItem,positionX, positionY, show, onPress, width, height } = this.props;
    ...
    const renderedItems = React.Children.map(items, (item) => {
      ...
      return (
        <TouchableWithoutFeedback  onPress={() => onPress(item.props.children, item.props.value) }>
          <View style={[{ padding: 10 },styleItem]}>
            <Text>{item.props.children}</Text>
          </View>
        </TouchableWithoutFeedback>
      );
    });
    var scale = listHeight?listHeight:3
    return (
      <View style={[styles.container, { top: positionY, left: positionX }]}>
        <ScrollView
          style={{ width: width - 2, height: height * scale }}
