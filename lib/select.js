...
class Select extends Component {
  ...
  _onPress() {
    const { optionListRef, children,styleItem, onSelect, width, height } = this.props;

    if (!children.length) {
      return false;
    }

    optionListRef()._show(children, styleItem,this.pageX, this.pageY, width, height, (item, value=item) => {
      if (item) {
        onSelect(value);
        this.setState({
          value: item
