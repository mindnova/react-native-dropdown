...
class Select extends Component {
  ...
  _onPress() {
    const { optionListRef, listHeight,children,styleItem, onSelect, width, height } = this.props;

    if (!children.length) {
      return false;
    }

    optionListRef()._show(children,listHeight, styleItem,this.pageX, this.pageY, width, height, (item, value=item) => {
      if (item) {
        onSelect(value);
        this.setState({
          value: item
